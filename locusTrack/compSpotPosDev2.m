function [spot] = compSpotPos(image, im_mask, MAX_NUM_SPOT, ...
    crop, channel, disp_flag, fig_num, opt)
...
im_loc  = uint8(imfilter( im, locusd, 'replicate' ));
im_loc_ = im_loc(mk);
thresh  = mean(im_loc_(logical(im_loc_)));
im_loc  = uint8(im_loc-thresh);
im_loc(~mk_for_fit) = 0;
...
for ii = iis
    ...
    mask_crop_for_fit = logical(double(imdilate(LL==i,sqr3)).*double(mk_for_fit));
