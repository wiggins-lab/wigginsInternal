...
function [spot] = compSpotPosDevFmin(image, ff, backer, im_mask, MAX_NUM_SPOT, ...
...
ddd = [max(image(im_mask))/max(backer(im_mask)),0];
...
im(crop+1:crop+imsize(1),crop+1:crop+imsize(2)) = ff;
...
im_loc_raw(crop+1:crop+imsize(1),crop+1:crop+imsize(2)) = ff;%imfilter( ff, gauss, 'replicate' );
